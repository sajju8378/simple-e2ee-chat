...
class ApiClient(private val baseUrl:String,private val token:String){
 ...
 data class Message(val id:String,val from:String,val to:String,val envelope:Map<String,Any?>,val createdAt:String,val status:String)
 ...
 companion object{
  ...
  fun login(base:String,id:String,password:String):AuthResult{val j=requestStatic(base,"POST","/v1/login",JSONObject().put("id",id).put("passwordHash",Crypto.passwordHash(password)));return AuthResult(j.getString("id"),j.optString("username",""),j.getString("token"),j.optString("publicKey",""),j.optString("displayName","User"),j.optString("keyBackup",""))}
