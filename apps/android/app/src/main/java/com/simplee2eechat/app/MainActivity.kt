...
class MainActivity : AppCompatActivity() {
    ...
    private fun baseLayout(titleText: String): LinearLayout = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL; setPadding(32, 42, 32, 28)
        addView(TextView(this@MainActivity).apply { text = titleText; textSize = 28f; gravity = Gravity.CENTER_HORIZONTAL }, LinearLayout.LayoutParams(-1, -2))
    }
    ...
    private fun showAuth(serverUrl: String) {
        poll = false; val root = baseLayout("Simple E2EE Chat")
        root.addView(TextView(this).apply { text = "Private one-to-one encrypted messaging"; textSize = 16f; gravity = Gravity.CENTER_HORIZONTAL; setPadding(0,8,0,24) })
        ...
        login.setOnClickListener {
            ...
            executor.execute { try {
                val r=ApiClient.login(base,uid,pw); val privateKey=store.privateKeyBlob() ?: error("This account's private key is not on this phone. Register this device again with a new Messenger ID.")
                store.saveAccount(r.id,r.token,privateKey,r.publicKey); api=ApiClient(base,r.token); main.post{showChat(r.id)}
            } catch(e:Exception){main.post{busy(false);note.text=e.message?:"Login failed"}} }
        }
        signup.setOnClickListener {
            ...
            executor.execute { try {
                val keys=Crypto.generateKeyPair(); val r=ApiClient.register(base,name,pw,keys.publicKey)
                store.saveAccount(r.id,r.token,keys.privateKey,r.publicKey); api=ApiClient(base,r.token)
                main.post{showChat(r.id);Toast.makeText(this,"Your Messenger ID is ${r.id}",Toast.LENGTH_LONG).show()}
            } catch(e:Exception){main.post{busy(false);note.text=e.message?:"Registration failed"}} }
        }
    }

    private fun showChat(myId:String){
        poll=true; val root=baseLayout("Messages")
        root.addView(TextView(this).apply{text="You: $myId";textSize=15f;setPadding(0,8,0,12)})
        val peer=edit("Friend's Messenger ID (E2E-XXXXXXXX)"); root.addView(peer)
        val open=Button(this).apply{text="Open conversation"}; root.addView(open)
        messagesBox=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(0,12,0,12)}
        root.addView(ScrollView(this).apply{addView(messagesBox);layoutParams=LinearLayout.LayoutParams(-1,0,1f)})
        status=TextView(this).apply{text="Choose a friend to start chatting.";setPadding(0,6,0,8)}; root.addView(status)
        messageInput=edit("Type an encrypted message…"); root.addView(messageInput)
        val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL}
        val send=Button(this).apply{text="Send securely";isEnabled=false}; val logout=Button(this).apply{text="Log out"}
        row.addView(send,LinearLayout.LayoutParams(0,-2,1f));row.addView(logout,LinearLayout.LayoutParams(0,-2,1f));root.addView(row);setContentView(root)
        open.setOnClickListener{
            currentPeer=peer.text.toString().trim().uppercase()
            if(!currentPeer.matches(Regex("E2E-[A-Z0-9]{8}"))){toast("Enter a valid Messenger ID");return@setOnClickListener}
            executor.execute{try{api?.getUser(currentPeer)?:error("Not logged in");main.post{send.isEnabled=true;status.text="Conversation with $currentPeer";loadMessages(myId)}}catch(e:Exception){main.post{status.text=e.message?:"Friend not found";send.isEnabled=false}}}
        }
        send.setOnClickListener{
            val text=messageInput.text.toString().trim();if(currentPeer.isBlank()||text.isBlank())return@setOnClickListener
            send.isEnabled=false;status.text="Encrypting and sending…"
            executor.execute{try{val a=api?:error("Not logged in");val recipient=a.getUser(currentPeer);val envelope=Crypto.encrypt(text,recipient.publicKey);val messageId=a.sendMessage(currentPeer,myId,envelope);sentPlaintext[messageId]=text;main.post{messageInput.setText("");status.text="Encrypted message sent ✓";send.isEnabled=true;loadMessages(myId)}}catch(e:Exception){main.post{status.text=e.message?:"Send failed";send.isEnabled=true}}}
        }
        logout.setOnClickListener{poll=false;store.clear();api=null;showAuth(getServerUrl())}
    }

    private fun loadMessages(myId:String){
        if(!poll||currentPeer.isBlank())return
        executor.execute{try{val list=api?.conversation(currentPeer)?:emptyList();val out=list.map{m->
