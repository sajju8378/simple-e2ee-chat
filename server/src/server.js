...
try { if (fs.existsSync(dataFile)) db = JSON.parse(fs.readFileSync(dataFile, 'utf8')); } catch {}
...
app.post('/v1/register', async (request, reply) => {
    ...
    const id = newId();
    db.users[id] = { id, displayName: displayName.trim(), password: passwordDigest(passwordHash), publicKey, createdAt: new Date().toISOString() };
    save();
    const token = crypto.randomBytes(32).toString('base64url');
    sessions.set(token, id);
    return reply.code(201).send({ id, token, publicKey, displayName: db.users[id].displayName });
});
...
app.post('/v1/login', async (request, reply) => {
    ...
    return { id: uid, token, publicKey: user.publicKey, displayName: user.displayName };
});
...
app.get('/v1/users/:id', async (request, reply) => {
    ...
    return { id: user.id, displayName: user.displayName, publicKey: user.publicKey };
});
...
app.post('/v1/messages', async (request, reply) => {
    ...
    if (!db.users[target]) return reply.code(404).send({ error: 'recipient not found' });
    const message = { id: crypto.randomUUID(), from: me, to: target, envelope, createdAt: new Date().toISOString() };
...
app.get('/v1/conversations/:peer', async (request, reply) => {
    ...
    if (!validId(peer) || !db.users[peer]) return reply.code(404).send({ error: 'user not found' });
    return { messages: db.messages.filter(m => (m.from === me && m.to === peer) || (m.from === peer && m.to === me)) };
});
